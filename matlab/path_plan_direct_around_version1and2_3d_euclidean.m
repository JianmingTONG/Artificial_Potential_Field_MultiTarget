function [ point ] = path_plan_direct_around_version1and2_3d_euclidean(infoGain, curr, target, obstacle, height, width)
    iters=1; %迭代次数
    dis = 10;
    path = [];
%     ever_been_times = 0;
    while ( (dis > 1))% &&  (iters<=2000))
        ...
        for i=1:number
            if (testPoint(1,i) < 1) || (testPoint(2,i) < 1) || (testPoint(1,i) > height) || (testPoint(2,i) > width)
                ...
                testOut(i) = computNewPotentialMultiGoal_version1and2_euclidean(infoGain, testPoint(:,i), target, obstacle, path);
            end
        end
        
        [temp num] = min(testOut);
        curr=testPoint(:,num);
        plot3(curr(1),curr(2),temp,'*r');
        fprintf("%d, potential %3.2f\n",iters, temp);
        path = [path, curr];
        
        for i = 1 : size(target, 2)
            tempDis = norm(curr-target(:,i));
            if dis > tempDis
                dis = tempDis;
            end
        end
